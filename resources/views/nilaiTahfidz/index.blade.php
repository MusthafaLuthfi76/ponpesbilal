...
@section('content')
    ...
    <div class="container-fluid p-3 p-md-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6 col-md-4">
                            <select name="semester" class="form-select" onchange="this.form.submit()">
                                ...
                                <option value="Ganjil" {{ request('semester') == 'Ganjil' ? 'selected' : '' }}>Semester
                                    Ganjil</option>
        ...
        <!-- Data Cards -->
        @if ($ujianList->count())
            @foreach ($ujianList as $item)
                @php
                    $santri = $item->santri;
                    $ta = $item->tahunAjaran;
                    $rep = $item->ujian_utama; // ujian representatif
                @endphp

                <div class="card border-0 shadow-sm mb-3 card-hover"
                    onclick="window.location.href='{{ route('nilaiTahfidz.show', [
                        'nis' => $santri->nis,
                        'tahun_ajaran_id' => $ta?->id_tahunAjaran,
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="d-flex align-items-center flex-wrap gap-1">
                                        <small class="text-muted">
                                            NIS: {{ $santri->nis ?? '-' }}
                                            @if ($rep?->penguji)
                                                <span class="ms-2">| Penguji: {{ $rep->penguji->nama }}</span>
                                            @endif
                                        </small>
                                        @if ($item->has_uts)
                                            <span class="badge bg-primary text-white px-2 py-1">UTS</span>
                                        @endif
                                        @if ($item->has_uas)
                                            <span class="badge bg-danger text-white px-2 py-1">UAS</span>
                                        @endif
                                        @if (!$item->has_uts && !$item->has_uas)
                                            <span class="badge bg-secondary text-white px-2 py-1">Belum Ujian</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="info-grid mb-3">
                                @if ($ta)
                                    <div class="info-item">
                                        ...
                                        <div>
                                            <span
                                                class="badge badge-custom {{ strtoupper($ta->semester) == 'GANJIL' ? 'semester-ganjil' : 'semester-genap' }} px-3 py-1">
                                ...
                                @if ($item->total_ujian > 0)
                                    <div class="info-item">
                                        <span class="info-label">Penguji</span>
                                        <span class="info-value">
                                            {{ optional($rep->penguji)->nama ?? '-' }}
                                        </span>
                                    </div>
                                @endif
                            </div>

                            <!-- Tombol Edit & Hapus (Mobile) -->
                            <div class="d-flex gap-2 mt-3" onclick="event.stopPropagation()">
                                <!-- Di bagian Mobile Layout -->
                                <button type="button" class="btn btn-sm btn-warning text-white flex-fill"
                                    onclick="event.stopPropagation(); editUjian(
        {{ $rep->id ?? 0 }},
        '{{ addslashes($santri->nama ?? '-') }}',
        '{{ addslashes($santri->nis ?? '-') }}',
        '{{ $ta?->id_tahunAjaran ?? '' }}',  <!-- Ganti ->id dengan ->id_tahunAjaran -->
        '{{ $rep?->id_penguji ?? '' }}'
    )">
                                ...
                                <button type="button" class="btn btn-sm btn-danger flex-fill"
                                    onclick="event.stopPropagation(); confirmDelete({{ $rep->id ?? 0 }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                        viewBox="0 0 24 24" fill="white">
                                ...
                                <a href="{{ route('nilaiTahfidz.show', [
                                    'nis' => $santri->nis,
                                    'tahun_ajaran_id' => $ta?->id_tahunAjaran,
                                    'semester' => $ta?->semester,
                                ]) }}"
                                    class="btn btn-sm {{ $item->has_uas ? 'btn-info' : ($item->has_uts ? 'btn-warning' : 'btn-success') }} text-white flex-fill"
                        ...
                        <div class="d-none d-lg-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    ...
                                    <div class="d-flex align-items-center gap-2 flex-wrap">
                                        <small class="text-muted">
                                            ...
                                            @if ($rep?->penguji)
                                                <span class="ms-2">
                                                    <strong>Penguji:</strong> {{ $rep->penguji->nama }}
                                                </span>
                                            @endif
                                        </small>
                            ...
                            @if ($ta)
                                <div class="text-center px-3">
                                    <span
                                        class="badge badge-custom {{ strtoupper($ta->semester) == 'GANJIL' ? 'semester-ganjil' : 'semester-genap' }} mb-1 px-3 py-2">
                                        Semester {{ $ta->semester }}
                                    </span>
                                    <div class="small text-muted fw-bold">{{ $ta->tahun }}</div>
                                </div>
                            @endif

                            <!-- Tombol Edit, Hapus, Lihat (Desktop) -->
        ...
        '{{ $rep?->id_penguji ?? '' }}'
                                ...
                                <button type="button" class="btn btn-danger btn-sm"
                                    onclick="event.stopPropagation(); confirmDelete({{ $rep->id ?? 0 }})" title="Hapus">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        viewBox="0 0 24 24" fill="white">
                                        <path fill="none" stroke="currentColor" stroke-linecap="round"
                                            stroke-linejoin="round" stroke-width="2"
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center py-5">
                    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24"
                        fill="none" stroke="#6c757d" stroke-width="1.5" class="mb-3">
                        <circle cx="12" cy="12" r="10"></circle>
                    ...
                    <p class="text-muted small mb-0">
                        @if (request()->filled('search') || request()->filled('semester') || request()->filled('tahun'))
                            Coba sesuaikan filter pencarian.
                        @else
                            Silakan tambahkan ujian baru.
                        @endif
                    </p>
                </div>
            </div>
    ...
    <!-- Modal Edit Ujian (Hanya Tahun Ajaran & Penguji) -->
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                ...
                <form id="formEditUjian" method="POST">
                    ...
                    <input type="hidden" id="edit_ujian_id" name="id">

                    <div class="modal-body">
                        <!-- Santri Info (Statik) -->
                        <div class="mb-3">
                            <label class="form-label fw-bold">Santri</label>
                            <div class="form-control bg-light" id="edit_santri_info" readonly>
                                <!-- Diisi via JS -->
                            </div>
                        </div>
                        ...
                        <!-- Tahun Ajaran -->
                        <div class="mb-3">
                            <label for="edit_tahun_ajaran_id" class="form-label fw-bold">Tahun Ajaran <span
                                    class="text-danger">*</span></label>
                            <select class="form-select" id="edit_tahun_ajaran_id" name="tahun_ajaran_id" required>
                                <option value="">Pilih Tahun Ajaran</option>
                                @foreach ($tahunList as $tahun)
                                    <option value="{{ $tahun->id_tahunAjaran }}">{{ $tahun->tahun }} –
                        ...
                        <!-- Penguji -->
                        <div class="mb-3">
                            <label for="edit_id_penguji" class="form-label fw-bold">Penguji <span
                                    class="text-danger">*</span></label>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-body text-center py-5">
                    ...
                    <p class="text-muted mb-0">Data ujian yang dihapus tidak dapat dikembalikan. Apakah Anda yakin ingin
                        melanjutkan?</p>
                </div>
    ...
    <script>
        function editUjian(id, namaSantri, nis, tahunAjaranId, idPenguji) {
            // Helper: aman dari null/undefined
            const safe = val => (val == null || val === 'null' || val === '') ? '' : String(val).trim();

            // Isi data santri (statik)
            const santriInfo = document.getElementById('edit_santri_info');
            if (santriInfo) {
                santriInfo.innerHTML =
                    `<strong>${safe(namaSantri)}</strong><br><small class="text-muted">NIS: ${safe(nis)}</small>`;
            }

            // Set dropdown
            const tahunSelect = document.getElementById('edit_tahun_ajaran_id');
            const pengujiSelect = document.getElementById('edit_id_penguji');
            const idInput = document.getElementById('edit_ujian_id');

            if (tahunSelect) tahunSelect.value = safe(tahunAjaranId);
            if (pengujiSelect) pengujiSelect.value = safe(idPenguji);
            if (idInput) idInput.value = id;

            // Set action form ke route PATCH yang benar
            const form = document.getElementById('formEditUjian');
            if (form) {
                // ✅ Route PATCH ke /nilaiTahfidz/ujian/{id}
                form.action = `/nilaiTahfidz/ujian/${encodeURIComponent(id)}`;
            }

            // Buka modal
            const modal = new bootstrap.Modal(document.getElementById('modalEditUjian'));
            modal.show();
        }

        function confirmDelete(id) {
            // ✅ Sesuaikan dengan route: DELETE /ujian/{id}
            const actionUrl = `/ujian/${encodeURIComponent(id)}`;

            const form = document.getElementById('formHapusUjian');
            if (form) {
                form.action = actionUrl;
                console.log('Hapus ujian dari index →', form.action);
            }

            const modal = new bootstrap.Modal(document.getElementById('modalHapusUjian'));
            modal.show();
        }

        // Submit filter form secara otomatis saat input berubah
