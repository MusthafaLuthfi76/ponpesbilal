...
@section('content')
                ...
                <div class="row g-3 g-md-4">
                        ...
                        <div class="d-flex align-items-center flex-wrap">
                            ...
                            <div class="mt-2 mt-sm-0">
                                @if ($tahunAjaranAktif)
                                    ...
                                    <div class="text-muted small mt-1 text-center">{{ $tahunAjaranAktif->tahun }}</div>
                                @else
                                    <span class="badge bg-secondary">Belum ada data ujian</span>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-sm-6 col-lg-3">
                                <div class="info-box">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        viewBox="0 0 24 24" fill="currentColor" class="mb-2">
                                        <path
                            ...
                            <div class="col-12 col-sm-6 col-lg-3">
                                <div class="info-box">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        viewBox="0 0 24 24" fill="currentColor" class="mb-2">
                                        <path
                                            d="M12 12a4 4 0 1 0-0.001-8.001A4 4 0 0 0 12 12zm6 8v-1.5c0-1.93-3.582-3.5-6-3.5s-6 1.57-6 3.5V20h12z" />
                                    </svg>
                            ...
                            <div class="col-12 col-sm-6 col-lg-3">
                                <div class="info-box">
                                    ...
                                    <div class="fw-semibold">{{ $santri->halaqah->nama_kelompok ?? 'Belum ditentukan' }}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            @if (isset($groupLabels) && count($groupLabels) > 1)
                                <form method="GET" action="{{ route('nilaiTahfidz.show', $santri->nis) }}"
                                    class="flex-grow-1" style="max-width: 300px;">
                            ...
                            @endif
                            <select class="form-select form-select-sm" style="max-width: 150px;" id="filterUjian"
                                onchange="filterData()">
                            ...
                            <button class="btn btn-primary btn-sm d-flex align-items-center gap-2 ms-auto"
                                data-bs-toggle="modal" data-bs-target="#modalTambahUjian">
                            ...
                            @forelse ($ujianList as $index => $ujian)
                                <tr class="ujian-row" data-jenis="{{ $ujian->jenis_ujian }}">
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge {{ $ujian->jenis_ujian == 'UTS' ? 'badge-uts' : 'badge-uas' }}">
                                            {{ $ujian->jenis_ujian }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge {{ $ujian->sekali_duduk == 'ya' ? 'bg-success' : 'bg-secondary' }}">
                                            {{ ucfirst($ujian->sekali_duduk) }}
                                        </span>
                                    </td>
                                    <td class="text-center"><strong
                                            class="text-primary">{{ $ujian->total_kesalahan }}</strong></td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <!-- TAMBAHKAN data-id-penguji dan data-tahun-ajaran-id -->
                                            <button class="btn btn-sm btn-warning btn-icon" title="Edit"
                                                data-id="{{ $ujian->id }}" data-jenis="{{ $ujian->jenis_ujian }}"
                                                data-juz="{{ $ujian->juz }}" data-tajwid="{{ $ujian->tajwid }}"
                                                data-itqan="{{ $ujian->itqan }}"
                                                data-sekali-duduk="{{ $ujian->sekali_duduk ?? 'tidak' }}"
                                                data-id-penguji="{{ $ujian->id_penguji }}"
                                                data-tahun-ajaran-id="{{ $ujian->tahun_ajaran_id }}"
                                                onclick="editUjianDetail(this)">
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger btn-icon" title="Hapus"
                                                data-id="{{ $ujian->id }}" onclick="confirmDeleteDetail(this)">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    viewBox="0 0 24 24" fill="white">
                                    ...
                                    <td colspan="8" class="text-center py-5">
                                        <div class="text-muted">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"
                                                viewBox="0 0 24 24" fill="currentColor" class="mb-2 opacity-50">
                                                <path
                    ...
                    @forelse ($ujianList as $index => $ujian)
                        <div class="mobile-card ujian-row" data-jenis="{{ $ujian->jenis_ujian }}">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    <span
                                        class="badge {{ $ujian->jenis_ujian == 'UTS' ? 'badge-uts' : 'badge-uas' }} me-2">
                                        {{ $ujian->jenis_ujian }}
                                    </span>
                                    <span
                                        class="badge {{ $ujian->sekali_duduk == 'ya' ? 'bg-success' : 'bg-secondary' }}">
                    ...
                    @empty
                        <div class="text-center py-5 px-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24"
                                fill="currentColor" class="mb-2 opacity-50 text-muted">
                                <path
                                    d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" />
                            </svg>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content modern-modal">
                ...
                <form action="{{ route('nilaiTahfidz.store') }}" method="POST">
                    ...
                    <!-- TAMBAHKAN HIDDEN INPUT OTOMATIS -->
                    <input type="hidden" name="tahun_ajaran_id" value="{{ $ujianPertama->tahun_ajaran_id ?? '' }}">
                    <input type="hidden" name="id_penguji" value="{{ $ujianPertama->id_penguji ?? '' }}">

                    <div class="modal-body">
                        <!-- Jenis Ujian -->
                        <div class="mb-3">
                            <label for="jenis_ujian" class="form-label fw-semibold">Jenis Ujian <span
                        ...
                        <!-- Juz -->
                        <div class="mb-3">
                            <label for="juz" class="form-label fw-semibold">Juz <span
                                    class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="juz" name="juz" min="1"
                                max="30" required>
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                <label for="tajwid" class="form-label fw-semibold">Kesalahan Tajwid <span
                                        class="text-danger">*</span></label>
                                <div class="input-group">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilai('tajwid', -1)">−</button>
                                    <input type="number" class="form-control text-center" id="tajwid" name="tajwid"
                                        value="0" min="0" required oninput="hitungTotal()">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilai('tajwid', 1)">+</button>
                                </div>
                            </div>
                            <div class="col-6">
                                <label for="itqan" class="form-label fw-semibold">Kesalahan Itqan <span
                                        class="text-danger">*</span></label>
                                <div class="input-group">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilai('itqan', -1)">−</button>
                                    <input type="number" class="form-control text-center" id="itqan" name="itqan"
                                        value="0" min="0" required oninput="hitungTotal()">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilai('itqan', 1)">+</button>
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control bg-light" id="total_kesalahan"
                                name="total_kesalahan" readonly>
                        </div>

                        <!-- Sekali Duduk -->
                        <div class="mb-3">
                            <label class="form-label fw-semibold d-block">Sekali Duduk <span
                                    class="text-danger">*</span></label>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content modern-modal">
                ...
                <form id="formEditUjian" method="POST">
                    ...
                    <!-- TAMBAHKAN HIDDEN INPUT UNTUK MENYIMPAN KONTEKS -->
                    <input type="hidden" name="id" id="edit_id">
                    <input type="hidden" name="nis" value="{{ $santri->nis }}">
                    <input type="hidden" name="tahun_ajaran_id" id="edit_tahun_ajaran_id">
                    <input type="hidden" name="id_penguji" id="edit_id_penguji">

                    <div class="modal-body">
                        <!-- Jenis Ujian -->
                        <div class="mb-3">
                            <label for="edit_jenis_ujian" class="form-label fw-semibold">Jenis Ujian <span
                                    class="text-danger">*</span></label>
                        ...
                        <!-- Juz -->
                        <div class="mb-3">
                            <label for="edit_juz" class="form-label fw-semibold">Juz <span
                                    class="text-danger">*</span></label>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                <label for="edit_tajwid" class="form-label fw-semibold">Kesalahan Tajwid <span
                                        class="text-danger">*</span></label>
                                <div class="input-group">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilaiEdit('edit_tajwid', -1)">−</button>
                                    <input type="number" class="form-control text-center" id="edit_tajwid"
                                        name="tajwid" value="0" min="0" required
                            ...
                            <div class="col-6">
                                <label for="edit_itqan" class="form-label fw-semibold">Kesalahan Itqan <span
                                        class="text-danger">*</span></label>
                                <div class="input-group">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilaiEdit('edit_itqan', -1)">−</button>
                                    <input type="number" class="form-control text-center" id="edit_itqan"
                                        name="itqan" value="0" min="0" required
                                        oninput="hitungTotalEdit()">
                                    <button type="button" class="btn btn-outline-success"
                                        onclick="ubahNilaiEdit('edit_itqan', 1)">+</button>
                                </div>
    ...
    <script>
        ...
        function editUjianDetail(buttonElement) {
            // Ambil data dari data attributes
            const id = buttonElement.dataset.id;
            const jenisUjian = buttonElement.dataset.jenis;
            const juz = buttonElement.dataset.juz;
            const tajwid = buttonElement.dataset.tajwid;
            const itqan = buttonElement.dataset.itqan;
            const sekaliDuduk = buttonElement.dataset.sekaliDuduk;
            // AMBIL DATA TAMBAHAN
            const idPenguji = buttonElement.dataset.idPenguji || '';
            const tahunAjaranId = buttonElement.dataset.tahunAjaranId || '';

            console.log('Edit Ujian Detail:', {
                id,
                jenisUjian,
                juz,
                tajwid,
            ...
            try {
                // Set nilai form
                const jenisSelect = document.getElementById('edit_jenis_ujian');
                if (jenisSelect) jenisSelect.value = jenisUjian;

                const juzInput = document.getElementById('edit_juz');
                if (juzInput) juzInput.value = juz;

                const tajwidInput = document.getElementById('edit_tajwid');
                if (tajwidInput) tajwidInput.value = tajwid;

                const itqanInput = document.getElementById('edit_itqan');
                if (itqanInput) itqanInput.value = itqan;

                const totalInput = document.getElementById('edit_total_kesalahan');
                if (totalInput) totalInput.value = parseInt(tajwid) + parseInt(itqan);

                // Set radio button untuk sekali duduk
                const radioYa = document.getElementById('edit_sekali_duduk_ya');
                const radioTidak = document.getElementById('edit_sekali_duduk_tidak');
                ...
                // SET NILAI HIDDEN INPUT KONTEKS
                const hiddenTahun = document.getElementById('edit_tahun_ajaran_id');
                if (hiddenTahun) hiddenTahun.value = tahunAjaranId;

                const hiddenPenguji = document.getElementById('edit_id_penguji');
                if (hiddenPenguji) hiddenPenguji.value = idPenguji;

                // Set action form
                const baseUrl = "{{ url('nilaiTahfidz') }}";
                const form = document.getElementById('formEditUjian');
                if (form) {
                    form.action = `${baseUrl}/${id}`;
                    console.log('Form action set to:', form.action);
                }

                // Hitung total untuk pertama kali
                hitungTotalEdit();

                // Show modal
                const modalElement = document.getElementById('modalEditUjian');
                if (modalElement) {
                    const modal = new bootstrap.Modal(modalElement);
                    modal.show();
                    console.log('Modal shown');
                } else {
                    console.error('Modal #modalEditUjian tidak ditemukan!');
                }
            } catch (error) {
                console.error('Error in editUjianDetail:', error);
                alert('Terjadi kesalahan saat membuka modal edit. Silakan coba lagi.');
            }
        }

        function confirmDeleteDetail(buttonElement) {
            const id = buttonElement.dataset.id;
            console.log('Delete Ujian:', id);

            const baseUrl = "{{ url('nilaiTahfidz') }}";
            const form = document.getElementById('formHapusUjian');
            if (form) {
                form.action = `${baseUrl}/${id}`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM loaded for detail page');

            // Test modal availability
            const editModal = document.getElementById('modalEditUjian');
            const deleteModal = document.getElementById('modalHapusUjian');
            const addModal = document.getElementById('modalTambahUjian');

            console.log('Modal elements found:');
            console.log('- Edit modal:', editModal ? 'Yes' : 'No');
            console.log('- Delete modal:', deleteModal ? 'Yes' : 'No');
            console.log('- Add modal:', addModal ? 'Yes' : 'No');
